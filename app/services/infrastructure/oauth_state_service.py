...
class OAuthStateService:
    ...
    async def _store_state(self, redis_key: str, user_id: str) -> bool:
        ...
        try:
            ...
            success = await fast_redis.set_with_ttl(redis_key, user_id, STATE_TTL_SECONDS)
    ...
    async def _retrieve_state(self, redis_key: str) -> str | None:
        ...
        try:
            # Use fast Redis client instead of HTTP calls
            result = await fast_redis.get(redis_key)
    ...
    async def get_state_owner(self, state: str) -> str | None:
        """Return the user ID associated with a state without deleting it."""
        if not state:
            return None
        redis_key = self._redis_key(state)
        return await self._retrieve_state(redis_key)
    ...
    async def _cleanup_state(self, redis_key: str) -> bool:
        ...
        try:
            # Use fast Redis client instead of HTTP calls
            success = await fast_redis.delete(redis_key)
...
async def get_oauth_state_owner(state: str) -> str | None:
    """Get the user ID associated with a state."""
    return await oauth_state_service.get_state_owner(state)
